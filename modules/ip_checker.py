    ...
    def get(RecordType:str="A") -> str:
        ...
        try:
            if RecordType == "A":
                ...
                error_message = f"无效的参数'{RecordType}',只能是'A'或'AAAA'"
    ...
    def v4() -> str:
        try:
            ...
            if response.status_code != 200:
                raise GetIPError(f"API 响应错误，错误代码：{response.status_code}")

            results = response.text
            if not ipaddress.ip_address(results):
                raise GetIPError(f"获取成功，但获取结果 {results} 不是有效的 IP 地址")

            public_ip = results
            if ipaddress.ip_address(results).version != 4:
                raise GetIPError(f"获取成功，但{public_ip}不能用于'A'记录")

            return public_ip

        except Exception as e:
            raise GetIPError(f'获取异常：{str(e)}')
    ...
    def v6() -> str:
        try:
            ...
            if ipaddress.ip_address(results).version != 6:
                raise GetIPError(f"获取成功，但{results}不能用于'AAAA'记录")
